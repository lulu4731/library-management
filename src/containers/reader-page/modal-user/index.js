        ...
        <>
            <Offcanvas show={isOpen} onHide={onClose} placement="end" scroll className="modal-love">
                ...
                <Offcanvas.Body className='pt-0'>
                                        ...
                                        <Col>
                                            ...
                                            >
                                                ...
                                                <Card.Body style={{ padding: "0px", width: '70%' }}>
                                                    ...
                                                    <Card.Text>
                                                        {/* Trạng thái: {JSON.parse(item.books).find((book) => book.borrow_status === 2) ?
                                                    'Chưa duyệt' : JSON.parse(item.books).find((book) => book.borrow_status === 4) ? 'Đã duyệt, đến lấy sách' : 'Đang mượn'} */}
                                                        Trạng thái:
                                                        {JSON.parse(item.books).find((book) => book.borrow_status === 2) && ' Chưa duyệt'}
                                                        {JSON.parse(item.books).find((book) => book.borrow_status === 4) && ' Đã duyệt, đến lấy sách'}
                                                        {JSON.parse(item.books).find((book) => book.borrow_status === 0) && ' Đang mượn'}
                                                        {/* {JSON.parse(item.books).find((book) => book.borrow_status === 1) && ' Đã trả'}
                                                        {JSON.parse(item.books).find((book) => book.borrow_status === 3) && ' Mất sách'} */}
                                                    </Card.Text>
                                                    <Card.Text>
                                                ...
                                                <Card.Footer
                                                    ...
                                                    ? 'Ngày lấy: ' + convertTimesTamp(JSON.parse(item.books).find((book) => book.borrow_status === 2)?.arrival_date || JSON.parse(item.books).find((book) => book.borrow_status === 4)?.arrival_date)
                                                    : 'Ngày trả: ' + convertTimesTamp(JSON.parse(item.books).find((book) => book.borrow_status === 0).expired)} */}
                                                        {/* {JSON.parse(item.books).find((book) => book.borrow_status === 2) || JSON.parse(item.books).find((book) => book.borrow_status === 4)
                                                    ? 'Ngày lấy: ' + convertTimesTamp(JSON.parse(item.books).find((book) => book.borrow_status === 2)?.arrival_date || JSON.parse(item.books).find((book) => book.borrow_status === 4)?.arrival_date)
                                                    : 'Ngày trả: ' + convertTimesTamp(JSON.parse(item.books).find((book) => book.borrow_status === 0).expired)} */}
                                                        {JSON.parse(item.books).find((book) => book.borrow_status === 2) && 'Ngày lấy: ' + convertTimesTamp(JSON.parse(item.books).find((book) => book.borrow_status === 2)?.arrival_date)}
                                                        {JSON.parse(item.books).find((book) => book.borrow_status === 4) && 'Ngày lấy: ' + convertTimesTamp(JSON.parse(item.books).find((book) => book.borrow_status === 4)?.arrival_date)}
                                                        {JSON.parse(item.books).find((book) => book.borrow_status === 0) && 'Ngày trả: ' + convertTimesTamp(JSON.parse(item.books).find((book) => book.borrow_status === 0)?.expired)}
                                                        {/* {JSON.parse(item.books).find((book) => book.borrow_status === 3) && 'Ngày báo mất sách: ' + convertTimesTamp(JSON.parse(item.books).find((book) => book.borrow_status === 3)?.date_return_book)} */}
                                                        {/* {JSON.parse(item.books).find((book) => book.borrow_status === 3) && 'Ngày báo mất sách: ' + convertTimesTamp(JSON.parse(item.books).find((book) => book.borrow_status === 3)?.date_return_book)} */}
                                                        {/* {JSON.parse(item.books).find((book) => book.borrow_status === 1) && 'Ngày trả sách: ' + convertTimesTamp(JSON.parse(item.books).find((book) => book.borrow_status === 1)?.date_return_book)} */}
                                                    </Card.Text>
                                                    {
                    ...
                    <h3 style={{ color: '#784cfb' }}>Danh sách tất cả các phiếu mượn đã hoàn thành</h3>
                    {
                        data.borrow?.length > 0 && data?.borrow.map((item, index) => {
                                        ...
                                        <Col>
                                            ...
                                            >
                                                ...
                                                <Card.Footer
                                                    style={{ borderTop: 0, paddingTop: '3%', width: '27%', backgroundColor: '#ffa0a0', borderTopRightRadius: 15, borderBottomRightRadius: 15 }}>
                                                    <Card.Text className='mb-2'>
                                                        {JSON.parse(item.books).find((book) => book.borrow_status === 1) && 'Ngày trả: ' + convertTimesTamp(JSON.parse(item.books).find((book) => book.borrow_status === 1)?.date_return_book)}
